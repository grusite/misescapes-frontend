<template>
  <form @submit.prevent="submitForm(userInfo)" class="flex flex-col space-y-5">
    <div v-if="hasName" class="flex flex-col space-y-1">
        <label for="email" class="text-sm font-semibold text-gray-500">Nombre</label>
        <input
            type="text"
            id="name"
            autofocus
    ...
    <div class="flex flex-col space-y-1">
        <div class="flex items-center justify-between">
            ...
            <nuxt-link to="#" class="text-sm text-blue-600 hover:underline focus:text-blue-800">olvidó la contraseña?</nuxt-link>
        </div>
        <input
            type="password"
            id="password"
